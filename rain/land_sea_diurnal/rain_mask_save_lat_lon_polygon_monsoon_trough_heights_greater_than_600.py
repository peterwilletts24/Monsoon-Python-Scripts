...
#  Get polygon intersect points for orography and ls masks 
...
# For Sea and Land, mask area and calculate mean of each hour for sea/land and SAVE as numpy array
    ...
    for s in ([1]):
     ...
     if s == 1:
          # Areas of land
         np.save('%s%s/%s/%s_land_rainfall_diurnal_np_domain_constrain_monsoon_trough_height_%s_%s_m' % (pp_file_path, expmin1, experiment_id, diag, height_min, height_max), trnp) 
