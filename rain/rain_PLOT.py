...
pcube_title='Est_Accumulate_Rain'
plot_diag='rain'
main_title='Total Accumulated Rainfall for EMBRACE period (est. from hourly means of instantaneous rainfall)'
#
# cmap= cm.s3pcpn_l
...
# experiment_ids = ['dkjxq', 'dklyu', 'dkmbq', 'dklwu', 'dklzq','dkbhu' ] #experiment_ids = ['djzny' ] 
 ...
 # Get min and max latitude/longitude and unrotate  to get min/max corners to crop plot automatically - otherwise end with blank bits on the edges 
  ...
  plt.savefig('/home/pwille/figures/%s/%s/%s%s_%s.png' % (experiment_id, plot_diag, pcube_title, experiment_id, plot_diag), format='png', bbox_inches='tight')
  plt.close()
