...
# For Sea and Land, mask area and calculate mean of each hour for sea/land and SAVE as numpy array
   ...
   #  total_rainfall = np.mean(maskedcube.reshape(maskedcube.shape[0], (maskedcube.shape[1]*maskedcube.shape[2])), axis=1)
    ...
    #     np.save('%s%s/%s/%s_sea_rainfall_diurnal_np_domain_constrain' % (pp_file_path, expmin1, experiment_id, diag), trnp)
         #iris.save(total_rainfall, '%s%s/%s/%s_seamask_diurnal_total.pp' % (pp_file_path, expmin1, experiment_id, diag))
    # if s == 1:
          # Areas of land
    #     np.save('%s%s/%s/%s_land_rainfall_diurnal_np_domain_constrain' % (pp_file_path, expmin1, experiment_id, diag), trnp)      
    ...
    np.save('%s%s/%s/%s_total_rainfall_diurnal_np_domain_constrain' % (pp_file_path, expmin1, experiment_id, diag), trnp.data.data) 
