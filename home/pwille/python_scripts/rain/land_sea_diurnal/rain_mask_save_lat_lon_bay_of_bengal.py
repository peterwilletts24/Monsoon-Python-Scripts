...
# For Sea and Land, mask area and calculate mean of each hour for sea/land and SAVE as numpy array
    ...
    for s in ([0]):
     ...
     if s == 0:
         ...
         np.save('%s%s/%s/%s_sea_rainfall_diurnal_np_domain_constrain_lat_%s-%s_lon-%s-%s_bay_of_bengal' % (pp_file_path, expmin1, experiment_id, diag, latmin, latmax, lonmin, lonmax), trnp)
