...
# For Sea and Land, mask area and calculate mean of each hour for sea/land and SAVE as numpy array
    ...
    for s in ([1]):
     ...
     #    print total_rainfall  
         #np.save('%s%s/%s/%s_sea_rainfall_diurnal_np_domain_constrain_lat_%s-%s_lon-%s-%s_ganga_basin' % (pp_file_path, expmin1, experiment_id, diag, latmin, latmax, lonmin, lonmax), trnp)
     ...
     if s == 1:
          # Areas of land
         np.save('%s%s/%s/%s_land_rainfall_diurnal_np_domain_constrain_ganga_basin' % (pp_file_path, expmin1, experiment_id, diag), trnp) 
