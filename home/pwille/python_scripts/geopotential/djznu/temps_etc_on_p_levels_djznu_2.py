...
def main():
 ...
 for experiment_id in experiment_ids:
  ...
  try:
      ...
      fname_heights = '/data/local/eepdw/%s/%s/15101_2.pp'% (expmin1, experiment_id)

      fname_temp = '/data/local/eepdw/%s/%s/4_2.pp' % (expmin1, experiment_id)
      fname_sp_hum= '/data/local/eepdw/%s/%s/10_2.pp' % (expmin1, experiment_id)
      path_tempfile='/data/local/eepdw' 


      load_name_i='/408_pressure_levels_interp_pressure_%s_2' % experiment_id

      save_name_temp='/temp_pressure_levels_interp_%s_2' % experiment_id
      save_name_sp_hum='/sp_hum_pressure_levels_interp_%s_2' % experiment_id
