...
def main():
 ...
 for experiment_id in experiment_ids:
  ...
  try:
      ...
      fname_heights = '/data/local/eepdw/%s/%s/15101_1.pp'% (expmin1, experiment_id)

      fname_temp = '/data/local/eepdw/%s/%s/4_1.pp' % (expmin1, experiment_id)
      fname_sp_hum= '/data/local/eepdw/%s/%s/10_1.pp' % (expmin1, experiment_id)
      path_tempfile='/data/local/eepdw' 


      load_name_i='/408_pressure_levels_interp_pressure_%s_1' % experiment_id

      save_name_temp='/temp_pressure_levels_interp_%s_1' % experiment_id
      save_name_sp_hum='/sp_hum_pressure_levels_interp_%s_1' % experiment_id
        ...
        for t, time_cube in enumerate(temp_cube.slices(['model_level_number', 'grid_latitude', 'grid_longitude'])):
            ...
            for f in range(lat_len):
                for y in range(lon_len):
                     ...
                     i_func_t = InterpolatedUnivariateSpline(heights[:,f,y], time_cube.data[:,f,y])
                     
                     p_hs=i['interps'][t,f,y,:]
                     te[f,y,:] = i_func_t(p_hs)       
      #with h5py.File(temp_p_heights_file, 'w') as tf:         
            temps[t,:,:,:] = te
            print 'Time %s saved' % t 
            sys.stdout.flush()
