...
for experiment_id in experiment_ids:
 ...
 glob_load = iris.load_cube(fg, 'surface_upward_latent_heat_flux' & time_constraint)
...
## Get time points from global LAM to use as time constraint when loading other runs
 ...
 heat = iris.load_cube(fu,'surface_upward_latent_heat_flux' & glob_tc)
 print heat
 ...
 for t, time_cube in enumerate (heat.slices(['time', 'grid_latitude', 'grid_longitude'])):
    #print time_cube
    ...
    #iris.save(time_cube.collapsed('time', iris.analysis.MEAN),'/projects/cascade/pwille/moose_retrievals/%s/%s/%s_mean.pp' % (expmin1, experiment_id, diag), append=True)
 del heat
